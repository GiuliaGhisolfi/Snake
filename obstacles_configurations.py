...
class Obstacles_configurations:
    ...
    def create_hamilton_cycle(self, grid):
        ...
        if (grid.x_blocks % 2) == 0:  # x_blocks are EVEN
            for i in range(grid.x_blocks-1): # slide through columns
                ...
                for j in range(grid.y_blocks-1):
                    if i % 2 == 1:  # odd column
                        ...
                        hamcycle[(i, grid.y_blocks-2-j)] = value
                    value += 1
            for i in range(grid.x_blocks):
                hamcycle[(grid.x_blocks-1-i, grid.y_blocks-1)] = value
                value += 1
            for j in range(grid.y_blocks-2):
                hamcycle[(0, grid.y_blocks-2-j)] = value
                value += 1

        else:  # x_blocks are ODD
            for i in range(grid.y_blocks):  # slide through rows
                for j in range(grid.x_blocks-1):
                    if i % 2 == 0:  # even rows
                        hamcycle[(j+1, i)] = value
                    else:           # odd rows
                        hamcycle[(grid.x_blocks-1-j, i)] = value
                    value += 1
            for i in range(grid.y_blocks-1):
                hamcycle[(0, grid.y_blocks-1-i)] = value
                value += 1
        return hamcycle
