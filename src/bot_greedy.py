...
class Bot_greedy(Bot):
    ...
    def graph_search(self, start, goal, graph):
        ...
        if self.chosen_search == 0:
            ...
            dummy = astar_search_min_turns(
                grid_problem, self.weights)  # min turns
            ...
            grid_problem = GridProblem(start, goal, graph, False)
            dummy = astar_search_saving_spaces(
                grid_problem, self.weights)  # saving spaces
    ...
    def optimize_standard_path(self):
        ...
        # return the number of the outgoing edges, connected with an edege not in "default path"
        ...
        if self.nnto == self.snake.get_body()[-1]:  # next node to optimize
            if is_optimizable(self.snake.get_body()[-1], tg):
                # find the next closer chokepoint
                chokepoint = None
                last_n = None
                ...
                if chokepoint != None:
                    ...
                    for choke_i in reversed(range(choke_ind + 1)):
                        ...
                        # without the head
                        patch = self.opt_search(
                            self.snake.get_body()[-1], chokepoint, true_g)
                        if patch != None:
                            self.default_path = patch + \
                                self.default_path[choke_i + 1:]
                            self.nnto = self.default_path[choke_i + 1]
                            return  # exit the for

            # calculate the next optimizable node
            for node in self.default_path[:-1]:
                if node in tg and is_optimizable(node, self.grid.grid):
                    # if we find the node, we save it and we exit the function
                    self.nnto = node
                    return
    ...
    def strategy(self):
        ...
        if computed_path_toFood != None:  # if it exist....
            ...
            next_pos = (
                snake_body + computed_path_toFood)[-len(snake_body) - 1:]
            ...
            if self.safe_cycle == 1:
                ...
                computed_cicle = self.graph_search(
                    start, goal, dummy_g)  # safe cycle

                if computed_cicle != None:
                    # if this also exists, we save the two paths and finish

                    self.default_path = computed_cicle + \
                        next_pos[1:]  # safe cycle
