...
class Bot_singleplayer(Player):
    ...
    def mela_cicle_strat(self):
        ...
        #computed_path_toFood = astar_search_saving_spaces(grid_problem).solution() #esiste il primo, per forza

        #ora calcoliamo dalla mela alla coda
        #aggiornando il grafo con la posizione futura
        next_pos = (self.prec_snake_body + computed_path_toFood)[-len(self.prec_snake_body) - 1:]
        ...
        # computed_cicle = astar_search_saving_spaces(grid_problem).solution() #esiste il primo, per forza
        ...
        move = graphDir_to_gameDir(self.prec_snake_body[-1], self.path_to_food[0]) # la mossa è stata presa, aggiorniamo
        ...
        while not self.termination:   
            ...
            if calcola:
                ...
                # search_tree = astar_search_saving_spaces(grid_problem) #esiste il primo, per forza
